...
impl Kline {
    ...
    pub fn collection_name(&self) -> String {
        format!("klines_{:?}_{:?}", self.market_type, self.interval)
    }

    /// 根据 market_type 和 interval 生成集合名称（静态方法）
    pub fn get_collection_name(market_type: MarketType, interval: KlineInterval) -> String {
        format!("klines_{:?}_{:?}", market_type, interval)
    }
    ...
    async fn ensure_collection_initialized(
        ...
        if exists {
            ...
            Self::create_indexes(market_type, interval).await?;
    ...
    pub async fn insert_one(&self) -> Result<mongodb::results::InsertOneResult, mongodb::error::Error> {
        ...
        // 确保集合已初始化
        Self::ensure_collection_initialized(&collection_name, self.market_type.clone(), self.interval.clone()).await?;
    ...
    pub async fn insert_many(klines: Vec<Kline>) -> Result<mongodb::results::InsertManyResult, mongodb::error::Error> {
        ...
        // 确保集合已初始化
        Self::ensure_collection_initialized(&collection_name, first_kline.market_type.clone(), first_kline.interval.clone()).await?;
    ...
    pub async fn find_one(
        ...
        let collection_name = Self::get_collection_name(market_type, interval);
    ...
    /// ```
    pub async fn create_indexes(market_type: MarketType, interval: KlineInterval) -> Result<(), mongodb::error::Error> {
        let Some(client) = MongoDBClient::global() else {
            return Err(mongodb::error::Error::custom("MongoDBClient 未初始化"));
        };
