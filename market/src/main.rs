...
use common::{AppConfig, MongoDBClient};
...
async fn main() {
    ...
    if config.mongodb.enabled {
        log::info!("初始化 mongodb client： {}", config.mongodb.url);
        MongoDBClient::init_global(
            &config.mongodb.url,
            &config.mongodb.database,
            config.mongodb.max_pool_size,
            config.mongodb.min_pool_size,
        ).await.unwrap()
    }
